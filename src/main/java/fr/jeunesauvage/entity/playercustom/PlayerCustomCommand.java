...
import fr.jeunesauvage.entity.playercustom.menu.Menu;
...
public class PlayerCustomCommand implements CommandExecutor {
    ...
    PlayerCustomCommand(EntityManager entityManager, MenuManager menuManager) {
    ...
    // add stat modifier
    private boolean handleMenu(CommandSender s, String[] args) {
        if (!(s instanceof Player sender)) return true;
        if (args.length > 1) {
            sender.sendMessage(Msg.msg("<red>Usage: /menu <player name/UUID>"));
            return true;
        }
        PlayerCustom    senderPlayer = PlayerCustomManager.getPlayerCustom(sender);
        Menu            menu;
        Player          target = null;
        if (args.length == 1)
        ...
        if (target == null) {
            // not found
            if (!(Bukkit.getEntity(UUID.fromString(args[0])) instanceof LivingEntity targetMob)) {
                sender.sendMessage(Msg.msg("<red>entity: <yellow><entity><red> is unknown", Msg.text("entity", args[0])));
                return true;
            }
            menu = new Menu(senderPlayer, targetMob, entityManager.getEntityModifierManager());
            menuManager.addMenu(senderPlayer, menu);
            return true;
        }
        // is player
        PlayerCustom    targetPlayer = PlayerCustomManager.getPlayerCustom(target);
        menu = new Menu(senderPlayer, targetPlayer);
        menuManager.addMenu(senderPlayer, menu);
        return true;
    }
	...
	private boolean handlePrintStats(CommandSender sender, String[] args) {
        ...
        PlayerCustom    playerCustom = PlayerCustomManager.getPlayerCustom(player);
        Print           print = new Print(playerCustom);
